...
//  Copyright (C) 2018-2021 Alessandro Lo Cuoco (alessandro.locuoco@gmail.com)
...
class Font
{
	...
	public:
		...
		Font(const char* FontFile, const unsigned int font_size, const int lscreen, const int hscreen, GLuint prog)
			: precCol(1,1,1,1), l(lscreen), h(hscreen), prog(prog), init(false)
		{
			if (!prog)
			{
				std::cerr << "Error: argument 'GLuint prog' in Font() not valid" << std::endl;
				return;
			}
			...
			trasf = glGetUniformLocation(prog, "Pmat");
			col = glGetUniformLocation(prog, "col");

			glm::mat4 m_Pmat = glm::ortho(0., (double)l, 0., (double)h);
			glUseProgram(prog);
			glUniformMatrix4fv(trasf, 1, GL_FALSE, &m_Pmat[0][0]);
		...
		// valid for string and wstring
		{
			...
			for (auto i = text.begin(); i != text.end(); ++i)
				switch (*i)
				{
					...
					case '\t':
					{
						for (unsigned int i = 0; i < l - margin; i += tab)
						{
							GLfloat t = xi + i;
							if (t > x)
							{

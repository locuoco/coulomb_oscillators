...
inline __host__ __device__ void p2p2_krnl(VEC *__restrict__ a, const int *mult, const int *index,
										  const VEC *__restrict__ p, int side, int radius,
										  int begi, int endi, int stride)
...
__global__ void p2p2(VEC *a, const int *mult, const int *index, const VEC *p, int m, int side, int radius)
{
	p2p2_krnl(a, mult, index, p, side, radius, blockDim.x * blockIdx.x + threadIdx.x, m, gridDim.x * blockDim.x);
}

void p2p2_cpu(VEC *a, const int *mult, const int *index, const VEC *p, int m, int side, int radius)
{
	std::vector<std::thread> threads(CPU_THREADS);
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(p2p2_krnl, a, mult, index, p, side, radius, niter*i, std::min(niter*(i+1), m), 1);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
}
...
__global__ void c2c2(Tree tree, int l, int radius = 1) // L -> 1
...
void appel(VEC *p, VEC *a, int n, const SCAL* param)
{
	...
	for (int l = L; l >= 2; --l)
		c2c2 <<< nBlocks, BLOCK_SIZE >>> (tree, l, radius);

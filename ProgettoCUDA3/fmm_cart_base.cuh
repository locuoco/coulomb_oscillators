...
// NOTE that if r2 >> EPS, it is also totally traceless, so it has just 2 independent elements
// for n >= 1 (just 1 for n = 0)
// r2 = r^2 = dot(d,d) + EPS
// this version works even for big EPS, in which case the gradient is not traceless
{
	if (n == 0)
...
// this version assumes r2 >> EPS which ease the computation significantly (from O(n^3) to O(n))

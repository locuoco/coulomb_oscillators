//  Fast multipole method (FMM) in cartesian coordinates
...
inline __host__ __device__ void fmm_c2c2_krnl(fmmTree tree, int l, int radius, SCAL d_EPS2,
...
__global__ void fmm_c2c2(fmmTree tree, int l, int radius, SCAL d_EPS2)
{
	extern __shared__ SCAL temp[]; // size must be at least (order+2)*blockDim.x
	SCAL *tempi = temp + (tree.p+2)*threadIdx.x;
	int n = tree_n(l);
	fmm_c2c2_krnl(tree, l, radius, d_EPS2, blockDim.x * blockIdx.x + threadIdx.x, n, gridDim.x * blockDim.x, tempi);
}

void fmm_c2c2_cpu(fmmTree tree, int l, int radius, SCAL d_EPS2)
{
	std::vector<std::thread> threads(CPU_THREADS);
	std::vector<SCAL*> temp(CPU_THREADS);
	for (int i = 0; i < CPU_THREADS; ++i)
	...
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(fmm_c2c2_krnl, tree, l, radius, d_EPS2, niter*i, std::min(niter*(i+1), m), 1, temp[i]);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
	for (int i = 0; i < CPU_THREADS; ++i)
		delete[] temp[i];
...
void fmm_cart(VEC *p, VEC *a, int n, const SCAL* param)
{
	...
	for (int l = L; l >= 2; --l)
		fmm_c2c2 <<< nBlocks, BLOCK_SIZE, smemSize >>> (tree, l, radius, EPS2);
...
void fmm_cart_cpu(VEC *p, VEC *a, int n, const SCAL* param)
{
	...
	if (coll)
		p2p2_cpu(a, tree.mult + beg, tree.index + beg, p, m, tree_side(L), radius, EPS2);
	else
		rescale_cpu(a, n, param+1);

	for (int l = L; l >= 2; --l)
		fmm_c2c2_cpu(tree, l, radius, EPS2);

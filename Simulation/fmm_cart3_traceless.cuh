...
#ifndef FMM_CART3_TRACELESS_CUDA_H
#define FMM_CART3_TRACELESS_CUDA_H

#include "fmm_cart3_symmetric.cuh"

inline __host__ __device__ void fmm_init3_traceless_krnl(fmmTree tree, int l, int begi, int endi, int stride)
{
	int offM = tracelessoffset3(tree.p+1);
...
__global__ void fmm_init3_traceless(fmmTree tree, int l)
{
	int m = tree_n(l);
	fmm_init3_traceless_krnl(tree, l, blockDim.x * blockIdx.x + threadIdx.x, m, gridDim.x * blockDim.x);
	//if (blockIdx.x == 0 && threadIdx.x == 0)
	//	printf("Inside fmm_init3\n");
}

void fmm_init3_traceless_cpu(fmmTree tree, int l)
{
	std::vector<std::thread> threads(CPU_THREADS);
	int m = tree_n(l);
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(fmm_init3_traceless_krnl, tree, l, niter*i, std::min(niter*(i+1), m), 1);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
}

inline __host__ __device__ void fmm_multipoleLeaves3_traceless_krnl(fmmTree tree, const VEC *p, int L,
...
__global__ void fmm_multipoleLeaves3_traceless(fmmTree tree, const VEC *p, int L)
{
	int m = tree_n(L);
	fmm_multipoleLeaves3_traceless_krnl(tree, p, L, blockDim.x * blockIdx.x + threadIdx.x, m, gridDim.x * blockDim.x);
	//if (blockIdx.x == 0 && threadIdx.x == 0)
	//	printf("Inside fmm_multipoleLeaves3\n");
}

void fmm_multipoleLeaves3_traceless_cpu(fmmTree tree, const VEC *p, int L)
{
	std::vector<std::thread> threads(CPU_THREADS);
	int m = tree_n(L);
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(fmm_multipoleLeaves3_traceless_krnl, tree, p, L, niter*i, std::min(niter*(i+1), m), 1);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
}

inline __host__ __device__ void fmm_buildTree3_traceless_krnl(fmmTree tree, int l, int begi, int endi, int stride, SCAL *tempi) // L-1 -> 0
...
__global__ void fmm_buildTree3_traceless(fmmTree tree, int l)
{
	extern __shared__ SCAL temp[]; // size must be at least (2*order+1)*blockDim.x
	SCAL *tempi = temp + (2*tree.p+1)*threadIdx.x;
	int n = tree_n(l);
	fmm_buildTree3_traceless_krnl(tree, l, blockDim.x * blockIdx.x + threadIdx.x, n, gridDim.x * blockDim.x, tempi);
	//if (blockIdx.x == 0 && threadIdx.x == 0)
	//	printf("Inside fmm_buildTree3\n");
}

void fmm_buildTree3_traceless_cpu(fmmTree tree, int l)
{
	std::vector<std::thread> threads(CPU_THREADS);
	...
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(fmm_buildTree3_traceless_krnl, tree, l, niter*i, std::min(niter*(i+1), m), 1, temp[i]);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
	for (int i = 0; i < CPU_THREADS; ++i)
		delete[] temp[i];
...
inline __host__ __device__ void fmm_c2c3_traceless_krnl(fmmTree tree, int l, int radius, SCAL d_EPS2,
...
__global__ void fmm_c2c3_traceless(fmmTree tree, int l, int radius, SCAL d_EPS2)
{
	extern __shared__ SCAL temp[]; // size must be at least (4*p+1)*blockDim.x
	SCAL *tempi = temp + (4*tree.p+1)*threadIdx.x;
	int n = tree_n(l);
	fmm_c2c3_traceless_krnl(tree, l, radius, d_EPS2, blockDim.x * blockIdx.x + threadIdx.x, n, gridDim.x * blockDim.x, tempi);
	//if (blockIdx.x == 0 && threadIdx.x == 0)
	//	printf("Inside fmm_c2c3\n");
}

void fmm_c2c3_traceless_cpu(fmmTree tree, int l, int radius, SCAL d_EPS2)
{
	std::vector<std::thread> threads(CPU_THREADS);
	std::vector<SCAL*> temp(CPU_THREADS);
	for (int i = 0; i < CPU_THREADS; ++i)
	...
	int niter = (m-1)/CPU_THREADS+1;
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i] = std::thread(fmm_c2c3_traceless_krnl, tree, l, radius, d_EPS2, niter*i, std::min(niter*(i+1), m), 1, temp[i]);
	for (int i = 0; i < CPU_THREADS; ++i)
		threads[i].join();
	for (int i = 0; i < CPU_THREADS; ++i)
		delete[] temp[i];
...
void fmm_cart3_traceless(VEC *p, VEC *a, int n, const SCAL* param)
{
	...
	for (int l = L; l >= 2; --l)
		fmm_init3_traceless <<< nBlocks, BLOCK_SIZE >>> (tree, l);
	...
	fmm_multipoleLeaves3_traceless <<< nBlocks, BLOCK_SIZE >>> (tree, p, L);

	smemSize = (2*tree.p+1)*BLOCK_SIZE*sizeof(SCAL);
	for (int l = L-1; l >= 2; --l)
		fmm_buildTree3_traceless <<< nBlocks, BLOCK_SIZE, smemSize >>> (tree, l);
	...
	for (int l = L; l >= 2; --l)
		fmm_c2c3_traceless <<< nBlocks, BLOCK_SIZE, smemSize >>> (tree, l, radius, EPS2);
...
void fmm_cart3_traceless_cpu(VEC *p, VEC *a, int n, const SCAL* param)
{
	int radius = tree_radius;
	...
	for (int l = L; l >= 2; --l)
		fmm_init3_traceless_cpu(tree, l);
	...
	fmm_multipoleLeaves3_traceless_cpu(tree, p, L);

	for (int l = L-1; l >= 2; --l)
		fmm_buildTree3_traceless_cpu(tree, l);
	...
	for (int l = L; l >= 2; --l)
		fmm_c2c3_traceless_cpu(tree, l, radius, EPS2);
...
#endif // !FMM_CART3_TRACELESS_CUDA_H
